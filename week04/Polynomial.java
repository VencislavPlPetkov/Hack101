	...
	public Polynomial<T> add(Polynomial<T> other) {
		...
		for (Map.Entry<Integer, T> entry : other.coefficients.entrySet()) {
			if (res.coefficients.containsKey(entry.getKey())) {
				res.coefficients.put(entry.getKey(), (T) (new Double(
						res.coefficients.get(entry.getKey()).doubleValue() + entry.getValue().doubleValue())));
			} else {
	...
	public String toString() {
		...
		for (Map.Entry<Integer, T> entry : coefficients.descendingMap().entrySet()) {
			...
			buffer.append(entry.getValue().toString())
					.append((entry.getKey() > 0) ? "x^" + entry.getKey().toString() : "").append(" ");
		}

		return buffer.substring(0, buffer.length() - 1);
	}

	public static <T extends Number> Polynomial<T> fromString(String polynomial) throws ParseException {
		...
		do {
			...
			if (poly.coefficients.containsKey(power)) {
				poly.coefficients.put(power,
						(T) (new Double(poly.coefficients.get(power).doubleValue() + coefficient.doubleValue())));
			} else {
